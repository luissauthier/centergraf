document.addEventListener("DOMContentLoaded", () => {
    ...
    */
    ...
    const { arteSvgText, facaSvgText } = await SVG_GEN.gerarSvgsCartela({
        ...
        personagemUrl,
        fixarImagem: true,
        fontUrl: '/assets/fonts/calibri-bold.ttf' // ajuste se mudar a fonte
    });
    ...
    SVG_GEN.downloadSvgAsPdf(arteSvgText, `${base}_ARTE.pdf`);
    // SVG_GEN.downloadText(`${base}_ARTE.svg`, arteSvgText);
    // SVG_GEN.downloadText(`${base}_FACA.svg`, facaSvgText);

    // abre Whats com texto (cliente anexa os 2 SVGs)
    const msgLinhas = [
        "🧾 *Pedido de etiquetas escolares — lhdigital (via Site)*",
        "",
